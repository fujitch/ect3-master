...
for i in range(training_epochs):
    for k in range(0, len(datasetTrain), batch_size):
        batch = np.zeros((batch_size, 98))
        batch = np.array(batch, dtype=np.float32)
        batch[:, 0] = datasetTrain[k:k+batch_size, 0]
        batch[:, 1:] = datasetTrain[k:k+batch_size, 2:]
    ...
    if i%display_epochs == 0:
        batch = np.zeros((datasetTrain.shape[0], 98))
        batch = np.array(batch, dtype=np.float32)
        batch[:, 0] = datasetTrain[:, 0]
        batch[:, 1:] = datasetTrain[:, 2:]
        output = datasetTrain[:, 1:2]
        train_loss = loss.eval(session=sess, feed_dict={x: batch, y_: output, keep_prob: 1.0})

        batch = np.zeros((datasetTest.shape[0], 98))
        batch = np.array(batch, dtype=np.float32)
        batch[:, 0] = datasetTest[:, 0]
        batch[:, 1:] = datasetTest[:, 2:]
    ...
    if i%1000 == 0:
        saver.save(sess, "./20181104DNNmodeldepth")
sess.close()
